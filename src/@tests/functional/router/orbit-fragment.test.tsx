...
Deno.test(
  ...
  async () => {
    ...
    try {
      const fullRes = await fetch('http://localhost:20801/orbit-fragment-fixture')
      ...
      const fragmentRes = await fetch('http://localhost:20801/orbit-fragment-fixture', {
      ...
      assertEquals(fullRes.headers.get('etag'), fragmentRes.headers.get('etag'))
    } finally {
  ...
  async () => {
    ...
    try {
      ...
      const fragmentResponse = await page.handleGet(
        mockHandlerContext({
          req: new Request('http://localhost/', { headers: { [ORBIT_FRAGMENT_HEADER]: '1' } }),
        }),
      )
...
Deno.test(
  ...
  async () => {
    ...
    try {
      ...
      const fullResponse = await page.handleGet(mockHandlerContext())
      const fullHtml = await fullResponse.text()
      assert(fullHtml.includes('<link rel="manifest" href="/manifest.webmanifest"'), fullHtml)
      assert(fullHtml.includes('<meta name="theme-color" content="#2563eb"'), fullHtml)
...
Deno.test(
  ...
  async () => {
    ...
    try {
      const fragmentRes = await fetch('http://localhost:20802/not-found-fixture', {
        headers: { [ORBIT_FRAGMENT_HEADER]: '1' },
      })
