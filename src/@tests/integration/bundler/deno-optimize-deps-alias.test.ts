  ...
  async () => {
    ...
    try {
      ...
      await withDevServer(root, async (server) => {
        ...
        assert(line?.includes('/.vite/deps/react.js'), `expected real optimized path, got: ${line}`)
      })
    } finally {
      await removeTempDirWithRetry(root)
    }
  ...
  async () => {
    ...
    try {
      ...
      await withDevServer(root, async (server) => {
        ...
        const result = await server.environments.client.transformRequest('/helper.ts')
  ...
  async () => {
    ...
    try {
      ...
      await withDevServer(root, (server) => {
        assert(server.config.environments.client.optimizeDeps.include?.includes('react'))
        assertFalse(
          server.config.environments.ssr.optimizeDeps.include?.includes('react'),
...
Deno.test(
  ...
  async () => {
    ...
    try {
      ...
      await withDevServer(root, async (server) => {
        const hasAlias = server.config.resolve.alias.some((entry: { find: unknown }) =>
          entry.find instanceof RegExp &&
          entry.find.test('definitely-not-a-real-package-xyz-123')
        )
        assertFalse(hasAlias, 'an unresolvable specifier must never get a real alias entry')
...
Deno.test(
  ...
  async () => {
    ...
    try {
      ...
      await Deno.writeTextFile(
        join(root, 'deno.json'),
        JSON.stringify({ imports: { ms: 'npm:ms@^2.1.3' }, nodeModulesDir: 'auto' }),
      )
      ...
      await withDevServer(root, async (server) => {
        assert(server.config.environments.client.optimizeDeps.include?.includes('ms'))
