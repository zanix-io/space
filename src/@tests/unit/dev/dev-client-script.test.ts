...
function runDevClientScript(
  ...
  const run = new Function('WebSocket', 'location', 'document', buildDevClientScript(options))
  run(FakeWebSocket, { ...location, reload: () => (reloaded = true) }, document)
...
Deno.test('buildDevClientScript: produces syntactically valid JavaScript', () => {
  ...
  new Function(buildDevClientScript({ routeFilePath: '/routes/products/page.tsx' }))
})
...
Deno.test('buildDevClientScript: connects to the reserved socket route, same-origin', () => {
  ...
  assertEquals(socket.url, `ws://localhost:3000/socket/${SPACE_DEV_SOCKET_ROUTE}`)
})

Deno.test('buildDevClientScript: uses wss:// when the page itself is https:', () => {
  const { socket } = runDevClientScript({}, { protocol: 'https:', host: 'example.com' })

  assertEquals(socket.url, `wss://example.com/socket/${SPACE_DEV_SOCKET_ROUTE}`)
})
...
Deno.test('buildDevClientScript: no routeFilePath reloads on any matching message', () => {
  ...
  run.socket.trigger(
    JSON.stringify({ kind: 'ssr-module-changed', affectedRoutes: ['/routes/unrelated/page.tsx'] }),
  )
  assert(run.reloaded)
})
...
Deno.test(
  ...
  () => {
    const run = runDevClientScript({ routeFilePath: '/routes/products/page.tsx' }, httpLocation)
...
Deno.test('buildDevClientScript: embeds the given routeFilePath verbatim', () => {
  const source = buildDevClientScript({ routeFilePath: '/routes/products/page.tsx' })
...
Deno.test(
  ...
  () => {
    const run = runDevClientScript({}, httpLocation, ['/app.css?direct', '/other.css?direct'])

    run.socket.trigger(JSON.stringify({ kind: 'client-css-changed', urls: ['/app.css?direct'] }))
...
Deno.test('buildDevClientScript: client-css-changed ignores a url with no matching link', () => {
  ...
  run.socket.trigger(
    JSON.stringify({ kind: 'client-css-changed', urls: ['/unrelated.css?direct'] }),
  )
