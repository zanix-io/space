/**
 * Hand-written `/@vite/client` replacement — shared infrastructure, NOT renderer-specific, despite
 * this module's earlier name (`dev-preact-hot-client.ts`) suggesting otherwise. Corrected after a
 * real, disposable spike (this package's own decision process) transformed a real React Comet
 * through the real engine and read the actual output: `spacePlugin()`'s `react()` branch (Rolldown's
 * native `oxc.jsx` transform, `refresh: true`) emits the EXACT SAME shape of `import.meta.hot` usage
 * `@prefresh/vite`'s Babel pass does for Preact —
 * ```
 ...
 * the top of any module that references it — confirmed for both renderers, not assumed for one from
 * the other.
 *
 ...
 * `dev-fast-refresh-preamble.ts` already established for React's own preamble: small, stable, and
 * confirmed against a real transform spike, not against documentation alone.
 *
 ...
 * `import.meta.hot` — not configurable, confirmed against `vite@8.2.1`'s own source before being
 * relied on here as a fixed constant rather than something this package resolves dynamically. */
 ...
 * transform needs them (confirmed against both real transform outputs). `invalidate(message)` is
 * NOT a no-op: React's own transform calls it when `RefreshRuntime.validateRefreshBoundaryAndEnqueueUpdate`
 * decides a module is no longer a valid refresh boundary (e.g. it stopped exporting only components)
 * — the real Vite client's own `invalidate` falls back to a full reload in that case, and this one
 * does the same, so an edit that can't be granularly applied still ends up correct instead of
 * silently stuck on stale code. Preact's own transform never calls it (confirmed against its real
 * output), so this only ever fires for React in practice today — harmless either way.
 *
 ...
 *
 * Hardened during this package's own Etapa 4 pass, after specifically probing this hand-written
 * client's own failure modes (not redesigning it — same shape, same message contract, same public
 * API):
 * - The cache-busting token must be `t=<EXACTLY 13 digits>` — nothing appended, nothing else in the
 *   query. Confirmed by reading Vite's own real source (`vite@8.2.1`,
 *   `dist/node/chunks/node.js`): `transformRequest()`'s very first line calls
 *   `removeTimestampQuery(url)`, whose regex is `/\bt=\d{13}&?\b/` — EXACTLY 13 digits, matched
 *   BEFORE `resolveId`/`load`/`transform` ever see the `id`. Vite's own real client (`client.mjs`)
 *   only ever builds this token as a bare server-side `Date.now()` (always 13 digits, nothing
 *   concatenated), so `removeTimestampQuery` always recognizes and strips it — the plugins
 *   (`@prefresh/vite`, `@vitejs/plugin-react`) and the transform's own `_jsxFileName`/
 *   `createHotContext(id)` embedding always see the CANONICAL, query-free id, even though the
 *   browser fetched a cache-busted URL. An earlier version of this token appended a monotonic
 *   in-page counter directly onto `Date.now()` (`Date.now() + '' + counter`, 14+ digits) to dodge a
 *   same-millisecond collision — pure digits, so it looked safe, but it no longer matches
 *   `\d{13}` exactly, so `removeTimestampQuery` never strips it, the query survives all the way to
 *   the plugins, and BOTH renderers' own refresh-wrapping is silently skipped for that request
 *   (`@prefresh/vite`'s `id.endsWith('.tsx')`-shaped check fails outright; React's own transform
 *   still wraps the module, but registers it under the query-suffixed id, which never matches the
 *   original mount's query-free id, so `RefreshRuntime` can't correlate the two) — confirmed
 *   empirically for both renderers via an isolated, `@zanix/space`-free repro (real npm Vite, real
 *   headless Chrome), not assumed. Fixed by keeping the token monotonic WITHOUT ever growing past
 *   13 digits: `nextHmrTimestamp()` below returns `Math.max(Date.now(), lastHmrTimestamp + 1)` —
 *   real wall-clock time on every call that isn't a same-millisecond repeat, and a strictly
 *   incrementing 13-digit value (still real milliscale, since it never runs meaningfully ahead of
 *   `Date.now()` in practice) on a same-millisecond repeat, which is exactly what a same-millisecond
 *   collision needs without ever appending a digit onto the value Vite itself expects to match.
 * - The whole re-import + `accept()` callback invocation is now wrapped in `try`/`catch`: a real
 *   syntax error in the edited file (the re-import itself rejects) or the `accept()` callback
 *   throwing synchronously (`RefreshRuntime.validateRefreshBoundaryAndEnqueueUpdate`, or Preact's
 *   own `flushUpdates()`, are both real functions this file doesn't control) previously left an
 *   unhandled promise rejection and applied nothing — this now falls back to a real reload, same
 *   philosophy `invalidate()` above already uses: a dev-only best-effort update failing to apply
 *   must still end up correct, never silently stuck on stale code.
 */
...
window.__spaceApplyClientUpdate = async function (url) {
  ...
  try {
    ...
    // breaks Fast-Refresh/Prefresh registration -- see this function's own doc for the real
    // regression this caused and how it was found.
